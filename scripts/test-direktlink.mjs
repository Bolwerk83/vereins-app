...
{ const felder=await page.locator('input[type="password"]').count();
  if(felder>0){
    ...
    if(/BITTE ANTWORTEN|ALS NÄCHSTES|DANACH|NÄCHSTES (TRAINING|SPIEL|TURNIER|TERMIN)|Kommt Ben/.test(b)) ok("Die Eltern landen direkt bei den Terminen ihres Kindes");
...
b=await body();
if(/BITTE ANTWORTEN|ALS NÄCHSTES|DANACH|NÄCHSTES (TRAINING|SPIEL|TURNIER|TERMIN)|Kommt Ben|Ben kommt/.test(b)) ok("Beim zweiten Mal geht es ohne jede Eingabe direkt zu den Terminen");
