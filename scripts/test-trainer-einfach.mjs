...
let b=await body();
if(/ALS NÄCHSTES|DANACH|NÄCHSTES (TRAINING|SPIEL|TURNIER|TERMIN)/.test(b)) ok("Der nächste Termin steht ganz oben und groß"); else fail("Kein Fokus-Termin: "+b.slice(0,200).replace(/\n/g," | "));
...
{ const oben=await page.evaluate(()=>{
    ...
    const l=[...document.querySelectorAll("div")].find(d=>/^(ALS NÄCHSTES|DANACH|NÄCHSTES (TRAINING|SPIEL|TURNIER|TERMIN)|AUSGEWÄHLTER TERMIN)/.test((d.innerText||"").trim()));
...
if(/BIS [A-ZÄÖÜ]+/.test(b)){
  const vorher=(b.match(/(?:ALS NÄCHSTES|DANACH|NÄCHSTES (?:TRAINING|SPIEL|TURNIER|TERMIN))[\s\S]{0,120}/)||[""])[0];
...
b=await body();
if(/ALS NÄCHSTES|DANACH|NÄCHSTES (TRAINING|SPIEL|TURNIER|TERMIN)/.test(b)) ok("Und zurück zur einfachen Sicht"); else fail("Rückweg fehlt");
