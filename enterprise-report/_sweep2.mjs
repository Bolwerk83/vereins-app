...
import fs from 'fs'
// Refactor-betroffen + repraesentative Stichprobe quer durch die Bereiche
...
const OUT = '/tmp/claude-0/-home-user-vereins-app/06740ba9-f1a4-550a-8eff-e1ccbf2c2bae/scratchpad/sweep2.txt'
...
for (const v of VIEWS) {
  ...
  await p.goto('http://localhost:4322', { waitUntil: 'commit' })
  await p.waitForTimeout(450)
  const txtLen = await p.evaluate(() => document.querySelector('main')?.innerText?.length || 0).catch(()=>0)
